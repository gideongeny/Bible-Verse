    ...
    private fun shareNote(card: View, note: FullNote) {
        ...
        notesMenu.setOnMenuItemClickListener { menuItem ->
            ...
            when (menuItem.itemId) {
                R.id.editNote -> {
                    findNavController().navigate(NotesFragmentDirections.actionAddEditNote(note.bibleId, note.chapterName, note.chapterId, note.verseIds.toIntArray(), note.id, note.comment))
                }
                R.id.shareNote -> {
